...
Tot nu toe geeft het fully-connected Neural Network met een 14x5x1-configuratie, een ReLU-activatiefunctie voor de hidden layer, een lineaire activatiefunctie voor de outputlayer, de 'Nadam' optimizer, een batchsize van 70 en 700 epochs de beste resultaten gebaseerd op de RMSE-metric (Validatie RMSE = 0.94). Met een iets complexer model dan het model uit milestone 2 is het dus gelukt om de gemiddelde afwijking van het actuele percentage positieve covid-19 tests te verlagen tot onder 1%. 
